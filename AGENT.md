...
- Arrival represents a complete work cycle from check-in to check-out. Review submission, break start, break extension, break completion, and starting the next focus session must not close, recreate, or reset an open arrival. Explicit check-out closes arrival records; the idle auto-checkout guard may also close an open arrival after continuous procrastination reaches the configured limit. Starting focus may auto-create an arrival only when none is open.
- Idle auto-checkout writes `arrival_sessions.left_at` to the effective business checkout time (`idleSince + maxDelayMinutes`) and writes `updated_at` to the actual database update time. Timeline, analytics, and manual-record overlap logic must never treat `updated_at` as a business time boundary.
