...
- 日历页面不参与秒级刷新，也不生成点阵。它从当前 snapshot 派生周/月读模型：周视图生成有效专注事件，月视图按日期聚合产物标签。
...
- 日历事件同样必须使用有效专注片段，而不是墙钟区间。具体规则与手动记录重叠校验一致：优先读取 `focus_segments`，并用 `actual_duration_minutes` 或计划时长封顶，避免浏览器后台延迟把事件画得过长。
