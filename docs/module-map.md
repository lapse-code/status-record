...
- Calendar 页面是另一种“事情回看”视图，不参与拖延/休息统计。周视图从已复盘 focus session 的有效专注区间生成事件卡片，显示时间、状态徽标、产物标签和文字描述；月视图按日期聚合并去重产物标签。
...
- 当侧栏数据操作区被响应式布局隐藏时，主内容顶部显示同样的“示例数据 / 清除示例 / 导入 / 导出”入口。
