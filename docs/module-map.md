...
| Arrival | 到岗记录、离开记录、拖延计算 | 到岗表示从到岗到离开的完整工作周期；只有离开关闭，到岗区间内未被专注、休息或不专注覆盖的等待时间是拖延 |
...
- Analytics 日视图显示当天统计、日点阵和当天记录明细；周/月视图只显示该范围聚合统计，不显示日点阵和记录明细。
- Analytics 时间占比使用方案 A：专注 + 不专注 + 拖延 = 100%，休息不进入分母；这三类时长与日点阵真实状态时长同源。
...
| Analytics | 统计复盘 | 日/周/月切换、时间占比、趋势图、标签分布、日视图记录明细 |
...
- Analytics 页面保留日、周、月统计；日视图展示日点阵和记录明细，周/月视图只展示范围聚合图表。
